...
export class SideMenuContentComponent {
	...
	public get subOptionIndentation(): number {
		if (this.platform.is('ios')) return Number(this.menuSettings.subOptionIndentation.ios);
		if (this.platform.is('windows')) return Number(this.menuSettings.subOptionIndentation.wp);
		return Number(this.menuSettings.subOptionIndentation.md);
	}

	// Get the proper height of each option
	// public get optionHeight(): number {
	// 	if (this.platform.is('ios')) return this.menuSettings.	.ios;
