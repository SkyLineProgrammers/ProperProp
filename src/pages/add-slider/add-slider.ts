...
/**
 * Generated class for the AddSliderPage page.
 *
...
@Component({
  selector: 'page-add-slider',
  templateUrl: 'add-slider.html',
})
export class AddSliderPage {
  ...
  constructor(public navCtrl: NavController, public navParams: NavParams, private database: AngularFireDatabase, public ObjGeneral: GeneralProvider, public loadingCtrl: LoadingController) {
    ...
    if (catId != undefined) {
      this.CatID = catId;
      this.database.object('FurnitureDB/SliderImages/' + catId).valueChanges().subscribe(data => {
        let SubArr = Object.keys(data);
        this.SliderName = data[SubArr[1]];
  ...
  AddCategory() {
    if (this.CatID == "") {
      if (this.SliderName != "") {
        this.database.list("FurnitureDB/SliderImages/").push({
          SliderName: this.SliderName,
          SliderImage: this.ImageLink
        });
        this.Reset();
      ...
      this.database.list("FurnitureDB/SliderImages/").update(this.CatID, {
        SliderName: this.SliderName,
        SliderImage: this.ImageLink
      })
      this.Reset();
  ...
  onFileChanged(param) {
    ...
    const StorageRef: firebase.storage.Reference = firebase.storage().ref('/photos/SliderImages/' + this.SliderName);
